...
def _plot_validation_data(
    ...
    # -------------------------------------------------------------------------
    breath_rate_resampled = resample_df(data.breath_rate_df, time_col='startDate')
    hr_resampled = resample_df(data.heart_rate_df, time_col='startDate')
    hrv_resampled = resample_df(data.hrv_df, time_col='startDate')
    ...
    sleep_df_copy['value'] = sleep_df_copy['value'].map(sleep_stage_map)
    ...
    save_path = f'/Users/ds/main/8sleep_biometrics/raw/plots/dump/{file_name}'
...
def analyze_predictions(data: DataManager, df_pred: pd.DataFrame, start_time: str, end_time: str, info: dict = None):
    ...
    data.heart_rate_df['startDate'] = pd.to_datetime(data.heart_rate_df['startDate'])
    column = 'heart_rate'
    ...
    for source, split_df in df_dict.items():
        ...
        results[source] = {
            ...
            'corr': f'{round(split_df_merged['value'].corr(split_df_merged['heart_rate']), 2) * 100:.2f}%',
            'mae': round(mean_absolute_error(split_df_merged['value'], split_df_merged['heart_rate']), 2),
            'mse': round(mean_squared_error(split_df_merged['value'], split_df_merged['heart_rate']), 2),
            'mape': f"{round(np.mean(np.abs((split_df_merged['value'] - split_df_merged['heart_rate']) / split_df_merged['value'])) * 100, 2)}%",
            'rmse': round(np.sqrt(mean_squared_error(split_df_merged['value'], split_df_merged['heart_rate'])), 2),
            'pre_merge_count': pre_merge_count,
            'post_merge_count': post_merge_count,
        }
        info['source'] = source
        print(json.dumps(results, indent=4))
        _plot_validation_data(start_time, end_time, data, split_df, info, results[source])
