...
def main():
    ...
    logger.debug(f"End time (UTC):   {args.end_time.isoformat()}")
    ...
    try:
        ...
        logger.debug(f"1 Memory Usage: {get_memory_usage_unix():.2f} MB")
        logger.debug(f"Free Memory: {get_free_memory_mb()} MB")
        ...
        del data
        gc.collect()
        logger.debug(f"2 Memory Usage: {get_memory_usage_unix():.2f} MB")
        logger.debug(f"Free Memory: {get_free_memory_mb()} MB")
        ...
        # Find baseline period to calibrate cap sensors

        merged_df = piezo_df.merge(cap_df, on='ts', how='inner')
        merged_df.drop_duplicates(inplace=True)
        logger.debug(f"3 Memory Usage: {get_memory_usage_unix():.2f} MB")
        logger.debug(f"Free Memory: {get_free_memory_mb()} MB")
        ...
        logger.debug(f"4 Memory Usage: {get_memory_usage_unix():.2f} MB")
        logger.debug(f"Free Memory: {get_free_memory_mb()} MB")
        ...
        logger.debug(f"5 Memory Usage: {get_memory_usage_unix():.2f} MB")
        logger.debug(f"Free Memory: {get_free_memory_mb()} MB")
